...
class Iamport {
  ...
  // 액세스 토큰 얻기
  async getToken({ imp_key, imp_secret }) {
  ...
  // 토큰 발급 & 결제 정보 얻기
  async getPaymentDataWithAccessToken({ imp_key, imp_secret, imp_uid }) {
    try {
  ...
  // 결제 완료 체크
  async completePayment({ imp_key, imp_secret, imp_uid, productAmount }) {
  ...
  // 카드 환불
  async cancelPayment({ imp_key, imp_secret, imp_uid, reason, cancelAmount }) {
    const paymentData = await this.getPaymentDataWithAccessToken({
      imp_key,
      imp_secret,
      imp_uid,
  ...
  async getCeritificationDataWithAccessToken({ imp_key, imp_secret, imp_uid }) {
    try {
      const access_token = await this.getToken({ imp_key, imp_secret });
      if (!access_token) {
        throw 'Invalid AccessToken';
